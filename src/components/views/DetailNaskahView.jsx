...
import { FileText, Calendar, Hash, ShieldAlert, CheckCircle2, User, ExternalLink, ShieldCheck, AlertCircle, History, ChevronDown, Check, Award } from 'lucide-react';
import { getRegistry, disputeClaimLocal, INITIAL_REGISTRY } from '../../lib/mockRegistry';
import { disputeParticipationOnChain, getExplorerTxUrl } from '../../lib/contract';
      ...
      {registryList.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-3xl p-8 sm:p-12 text-center shadow-card space-y-4">
          ...
          <p className="text-xs sm:text-sm text-slate-600 max-w-md mx-auto leading-relaxed">
            The decentralized registry is currently empty. Register your paper abstract first to generate cryptographic hashes and anchor an immutable timestamp on the blockchain.
          </p>
          {setActiveTab && (
            <div className="pt-2">
              <button
        ...
        <>
          ...
          <div className="relative flex-1 sm:w-80">
            <button
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-xs font-medium text-slate-800 flex items-center justify-between gap-2 text-left transition-all focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
            >
            ...
            {dropdownOpen && (
              <div className="absolute left-0 top-full mt-1.5 w-full sm:w-[380px] bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden py-1.5 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  {registryList.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-3.5 py-2.5 text-xs transition-colors flex items-center justify-between gap-2 ${
                          isSelected
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <p className={`text-xs truncate ${isSelected ? 'text-brand-700 font-semibold' : 'text-slate-800'}`}>
                            {item.title}
                          </p>
        ...
        {currentManuscript && (
          <div className="text-xs font-mono text-slate-500">
            {isOwner ? (
              ...
              <span className="text-slate-400">Public Preview Mode</span>
            )}
          </div>
        )}
      </div>
